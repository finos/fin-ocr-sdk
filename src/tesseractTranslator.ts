...
export class TesseractTranslator implements Translator {
    ...
    private initFonts() {
        ...
        for (const font of fonts) {
            const trainedData = this.ocr.root.readFile(`${font}.traineddata`).toString();
    ...
    public async translateImage(img: Image, opts?: TranslatorOpts): Promise<TranslatorResult> {
        const ctx = img.ctx;
        opts = opts || {};
        ...
        const chars: TranslatorChar[] = debug ? this.toChars(data.symbols, img, ctx) : [];
        this.releaseWorker(worker);
        if (ctx.isDebugEnabled()) ctx.debug(`tesseract result: score=${score}, value=${value}`);
    ...
    private async terminateWorkers(ctx: Context) {
        if (ctx.isDebugEnabled()) ctx.debug(`Terminating ${this.workers.length} tesseract workers`)
        for(;;) {
            const worker = this.workers.pop();
            if (!worker) break;
            await worker.terminate();
        }
        if (ctx.isDebugEnabled()) ctx.debug(`Terminated ${this.workers.length} tesseract workers`)
    }

    private toChars(symbols: Symbol[], inputImage: Image, ctx: Context): TranslatorChar[] {
        if (ctx.isDebugEnabled()) ctx.debug(`toChars enter: ${symbols.length} symbols, image size: ${JSON.stringify(inputImage.mat.size())}`);
